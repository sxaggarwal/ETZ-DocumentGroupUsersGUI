...
class DocGroupAccess(tk.Tk):
    def __init__(self):
        ...
        self.current_user_display_list = self.user_display_list.copy()  # Store current display list
    ...
    def make_combobox(self):
        ...
        self.enabled_user_checkbox = tk.Checkbutton(self, text='Enabled Users', variable=self.enabled_user_var, command=self.filter_by_enabled_user)
        self.enabled_user_checkbox.grid(row=0, column=2)

        tk.Label(self, text="Select Document Group(s):").grid(row=1, column=0)
        self.document_group_listbox = tk.Listbox(self, height=10, width=50, exportselection=False, selectmode=tk.MULTIPLE)
        ...
        self.selected_user_info = tk.Listbox(self, height=10, width=50, exportselection=False, selectmode=tk.MULTIPLE)
        ...
        remove_access_button = tk.Button(self, text="REMOVE ACCESS", command=self.confirm_delete_access)
        remove_access_button.grid(row=3, column=1)
        remove_all_access_button = tk.Button(self, text="REMOVE ALL ACCESS", command=self.confirm_remove_all_access)
    ...
    def filter_by_enabled_user(self):
        ...
        self.user_display_list = [f"{name} (UserPK: {pk})" for name, pk in user_data_list]
    ...
    def give_access(self):
        ...
        if not selected_indices:
            messagebox.showwarning("No Selection", "Please select at least one document group.")
            return
        ...
        for index in selected_indices:
            ...
            if selected_doc_group_pk and self.user_pk:
                self.database_conn.add_document_group_user(selected_doc_group_pk, self.user_pk)
    ...
    def confirm_delete_access(self):
        ...
        confirmation = messagebox.askyesno("Confirm Delete", "Are you sure you want to delete selected access?")
        if confirmation:
            self.delete_access()
    ...
    def confirm_remove_all_access(self):
        ...
        confirmation = messagebox.askyesno("Confirm Remove All", "Are you sure you want to remove all access?")
        if confirmation:
